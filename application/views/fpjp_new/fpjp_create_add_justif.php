		...
		<div class="col-sm-6">
	        ...
	        <div id="justif" class="form-group m-b-10 d-none">
	            ...
	            <div class="col-sm-1 col-md-1">
	            	<a href="javascript:void(0)" class="font-size-14 mt-5 d-block" title="Add New Justif" id="add_justif" style="color:#01a2cc !important"><i class="fa fa-plus"></i></a>
	            </div>
	        </div>
	        <div id="justif_2" class="form-group m-b-10 d-none">
	            <label for="fs_header_2" class="col-sm-3 control-label text-left">&nbsp; <span class="pull-right">&nbsp;</span></label>
	            <div class="col-sm-8 col-md-5">
	            	<select class="form-control input-sm" id="fs_header_2">
	            		<option value="0">-- Choose --</option>
		            </select>
	            </div>
	            <div class="col-sm-1 col-md-1">
	            	<a href="javascript:void(0)" class="font-size-14 mt-5 d-block" title="Add New Justif" id="rm_justif_1" style="color:#01a2cc !important"><i class="fa fa-trash"></i></a>
	            </div>
	        </div>
	        <div id="justif_3" class="form-group m-b-10 d-none">
	            <label for="fs_header_3" class="col-sm-3 control-label text-left">&nbsp; <span class="pull-right">&nbsp;</span></label>
	            <div class="col-sm-8 col-md-5">
	            	<select class="form-control input-sm" id="fs_header_3">
	            		<option value="0">-- Choose --</option>
		            </select>
	            </div>
	            <div class="col-sm-1 col-md-1">
	            	<a href="javascript:void(0)" class="font-size-14 mt-5 d-block" title="Add New Justif" id="rm_justif_2" style="color:#01a2cc !important"><i class="fa fa-trash"></i></a>
	            </div>
	        </div>
	        <div class="form-group m-b-10">
   ...
   <?= 'let data_bank = '. json_encode($all_bank) . ';'; ?>
	...
	$("#vendor_name").on("change", function(){
	    ...
	    for (var i=0; i < arr_bank.length; i++) {
	    	bank = arr_bank[i];
				opt_bank += '<option value="' + bank.ACCT_NUMBER + '" data-bank="' + bank.NAMA_BANK + '" data-rek="' + bank.NAMA_REKENING + '" data-acct="' + bank.ACCT_NUMBER + '">' + bank.NAMA_BANK + ' - ' + bank.NAMA_REKENING + '</option>';
			}
	...
	let url          = baseURL + 'fpjp/api/load_data_fs_custom';
          ...
          "drawCallback": function ( settings ) {
          	// if(id_fs > 0){
	            ...
	            row_datas.each(function (data, i) {
	            	list_fs_rkap.push({ "id_fs": data.id_fs,  "id_rkap": data.id_rkap });
...
/*
		    ...
		    data = {
								list_fs_rkap : list_fs_rkap,
								id_fs : id_fs,
								id_fs_new_1 : id_fs_new_1,
								id_fs_new_2 : id_fs_new_2,
		    ...
		    $.ajax({
		        url   : baseURL + 'fpjp/api/save_fpjp_custom',
		        type  : "POST",
		        data  : data,
		        dataType: "json",
		        beforeSend  : function(){
	...
	$("#fs_header_2").bind("click", function(e) {
	    lastValue = $(this).val();
	}).bind("change", function(e) {
		change = true;
		if(change == true){
			id_fs_1 = $(this).val();;
			id_fs_new_1 = id_fs_1;
console.log(list_fs_rkap);
			$.ajax({
	        url   : baseURL + 'fpjp/api/load_data_fs_custom',
	        type  : "POST",
	        data  : {id_fs : id_fs_1},
	        dataType: "json",
	        success : function(result){
	        	...
	        	if(total > 0){
	        		...
	        		for(var i = 0; i < data.length; i++) {
	        			obj = data[i];
	        			// fund_av_add += parseInt( obj.fund_av_amount );
								fund_av_add += parseInt( obj.fund_av_amount.replace(/\./g, '') );
	            	list_fs_rkap.push({ "id_fs": obj.id_fs,  "id_rkap": obj.id_rkap });
	        		}
	        		table.rows().eq(0).each( function ( index ) {
								j = index+1;
								fund_av_b = parseInt( $("#fund_av-"+j).val().replace(/\./g, '') );
								count_fund_av = fund_av_b + fund_av_add;
	        			$("#fund_av-"+j).val( formatNumber(count_fund_av) );
							});
						}
	...
	$("#fs_header_3").bind("click", function(e) {
	    lastValue = $(this).val();
	}).bind("change", function(e) {
		change = true;
		if(change == true){
   ...
   function getDivision() {
	...
	let store_data_justif = [];

	function getFS(new_justif=false) {
		...
		if(new_justif !== false){
			...
			let el = $("#fs_header_"+new_justif);

			el.attr('disabled', true);
			el.css('cursor', 'wait');

			let justif2 = store_data_justif;
  		let fs_header_2 = opt_default;

			let id_fs_1 = $("#fs_header").val();
			let id_fs_2 = "";
  		if(new_justif == 3){
  			id_fs_2 = $("#fs_header_2").val();
  		}
    	for(var i = 0; i < store_data_justif.length; i++) {
			    obj = store_data_justif[i];

			    if(obj.id_fs != id_fs_1 && obj.id_fs != id_fs_2){
			    	fs_header_2 += '<option value="'+ obj.id_fs +'" data-currency="'+ obj.currency +'" data-cadangan="'+ obj.is_cadangan +'" data-rate="'+ obj.rate +'">'+ obj.fs_number +' - '+ obj.fs_name +'</option>';
			    }
			}
	    ...
	    $.ajax({
	        ...
	        success : function(result){
	        	...
	        	if(parse.status == true){
		        	...
		        	for(var i = 0; i < data.length; i++) {
					    obj = data[i];
					    fs_header += '<option value="'+ obj.id_fs +'" data-currency="'+ obj.currency +'" data-cadangan="'+ obj.is_cadangan +'" data-rate="'+ obj.rate +'">'+ obj.fs_number +' - '+ obj.fs_name +'</option>';

					    store_data_justif.push({"id_fs": obj.id_fs, "currency": obj.currency, "is_cadangan": obj.is_cadangan, "rate": obj.rate, "fs_number": obj.fs_number, "fs_name": obj.fs_name});
					}
	        	}
  ...
  /*  success: function (response) {
   ...
   function getTax(id_row, id_rkap, id_select=0) {
	...
	$("#add_justif").on("click", function(e) {
			let justif_2 = $("#justif_2");
			let justif_3 = $("#justif_3");


			if ( justif_2.hasClass("muncul") == true && justif_3.hasClass("muncul") == false) { 
				$("#justif_3").addClass('muncul');
				$("#justif_3").removeClass('d-none');
				getFS(3);
			}

			if ( justif_2.hasClass("muncul") == false) { 
				$("#justif_2").addClass('muncul');
				$("#justif_2").removeClass('d-none');
				getFS(2);
			}
/*
			if()else{
				// $("#add_justif").addClass('d-none');
				getFS(3);
			}*/
	});
	$("#rm_justif_1").on("click", function(e) {
			$("#justif_2").addClass('d-none');
			$("#justif_2").removeClass('muncul');
			// $("#add_justif").removeClass('d-none');
			table.draw();
	});

	$("#rm_justif_2").on("click", function(e) {
			$("#justif_3").addClass('d-none');
			$("#justif_3").removeClass('muncul');
			// $("#add_justif").removeClass('d-none');
			table.draw();
	});
