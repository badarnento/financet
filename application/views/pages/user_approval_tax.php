...
<script>
  ...
  $(document).ready(function(){
    ...
    let url  = baseURL + 'user_approval_tax/load_data';
    ...
    Pace.track(function(){
      $('#table_data').DataTable({
        ...
        { "data": "pph", "width": "300px", "class": "text-left" },
        ...
        "autoWidth"       : true,
        "bAutoWidth"      : true,
        "rowsGroup"       : [17, 18, 19, 20, 21, 22, 23],
    ...
    $("#btndownloadbt").on("click", function(){
     ...
     let url   ="<?php echo site_url(); ?>user_approval_tax/download_data_before_tax";
    ...
    $("#btndownloadat").on("click", function(){
     ...
     let url   ="<?php echo site_url(); ?>user_approval_tax/download_data_after_tax";
    ...
    $("#save_data").on('click', function () {
      ...
      for (var i = 0; i < total_detail; i++) {
        ...
        wht_pph       = $("#unit_opt-"+header_id).find(':selected').attr('wht-pph');
        ...
        mst_ppn       = $("#unit_optppn-"+header_id).find(':selected').attr('mst-ppn');
        ...
        data_line.push({'journal_name' : journal_name, 'gl_pph' : gl_pph, 'persen_pph' : persen_pph, 'wht_pph' : wht_pph, 'gl_ppn' : gl_ppn, 'persen_ppn' : persen_ppn, 'mst_ppn' : mst_ppn, 'detail_data' : detail_data_all[i], 'validated' : validated, 'npwp' : npwp, 'faktur_pajak' : faktur_pajak, 'tgl_faktur_pajak' : tgl_faktur_pajak, 'amount_base_fee' : amount_base_fee, 'notes' : notes, 'header_id' : header_id});
      }
      ...
      $.ajax({
        url   : baseURL + 'user_approval_tax/save_data',
        type  : "POST",
        data  : data,
        beforeSend  : function()
          {
        ...
        success : function(result){
          ...
          if(result.status == true){
            ...
            customLoading('hide');
            customNotif('Error','Save Failed, Please Fill Out All Field!','error' );
          }
        }
      });
    });
    ...
    let url1  = baseURL + 'user_approval_tax/load_data_after';
        ...
        drawCallback: function (settings) {
          ...
          api.column(6, { page: 'current' }).data().each(function (group, i) {
            ...
            if (last != group && i > 0) {
              ...
              $(rows).eq(i).before(
                '<tr class="group"><td align="center" colspan="17" style="color:#fff;background-color:#999;line-height: 0px;height: 5px;padding: 0px;">&nbsp;</td></tr>'
                );
            }
            last = group;
          });
