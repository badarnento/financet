          ...
          <table width="100%" class="table display table-bordered table-striped table-responsive hover" id="table_data"> 
            <thead>
              <tr>
                ...
                <th class="text-center">Wht Tax Type</th>
                <th class="text-center">Wht Des</th>
                <th class="text-center">Wht Tax Code</th>
                <th class="text-center">Wht Tax Code Des</th>
      ...
      <form role="form" id="form-edit" data-toggle="validator">
        ...
        <div class="modal-body"> 
          <div class="row">
            <div class="form-group">
              <div class="col-lg-6">
                <label class="form-control-label" for="lblWhtTaxType">Wht Tax Type</label>
                <input type="text" class="form-control" id="txtWhtTaxType" name="txtWhtTaxType" placeholder="Wht Tax Type" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                <div class="help-block with-errors"></div>
                <input type="hidden" name="isNewRecord" id="isNewRecord" val="0">
                <input type="hidden" name="id_wht_tax" id="id_wht_tax">
              </div>
              <div class="col-lg-6">
                <label class="form-control-label" for="lblWhtDesc">Wht Desc</label>
                <input type="text" class="form-control" id="txtWhtDesc" name="txtWhtDesc" placeholder="Wht Desc" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                <div class="help-block with-errors"></div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="form-group">
              <div class="col-lg-6">
                <label class="form-control-label" for="lblWhtTaxCode">Wht Tax Code</label>
                <input type="text" class="form-control" id="txtWhtTaxCode" name="txtWhtTaxCode" placeholder="0" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                <div class="help-block with-errors"></div>
              </div>
              <div class="col-lg-6">
                <label class="form-control-label" for="lblWhtTaxCodeDesc">Wht Tax Code Desc</label>
                <input type="text" class="form-control" id="txtWhtTaxCodeDesc" name="txtWhtTaxCodeDesc" placeholder="Wht Tax Code Desc" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                <div class="help-block with-errors"></div>
              </div>
            </div>
          </div>
...
<script>
  $(document).ready(function () {
    ...
    let url = baseURL + 'master/load_data_pph';
    let ajaxData = {
      "url"  : url,
      "type" : "POST"
    }
    ...
    { "data": "wht_tax_type", "width":"100px", "class":"text-left"},
    { "data": "wht_desc", "width":"200px", "class":"text-left"},
    { "data": "wht_tax_code", "width":"100px", "class":"text-left"},
    { "data": "wht_tax_code_desc", "width":"200px", "class":"text-left"},
   ...
   $('#table_data').on( 'click', 'a.action-edit', function () {
    ...
    $("#isNewRecord").val("0");
    $("#id_wht_tax").val(data.id_wht_tax);
    $("#txtWhtTaxType").val(data.wht_tax_type);
    $("#txtWhtDesc").val(data.wht_desc);
    $("#txtWhtTaxCode").val(data.wht_tax_code);
    $("#txtWhtTaxCodeDesc").val(data.wht_tax_code_desc);
    $("#txtPercentage").val(data.percentage);
    $("#txtGlAccount").val(data.gl_account);
    $("#modal-edit-label").html('Edit : ' +  data.wht_tax_type + '-' + data.wht_tax_code);
   ...
   $('#table_data ').on( 'click', 'a.action-delete', function () {
    ...
    data      = table.row( $(this).parents('tr') ).data();
    id_delete = data.id_wht_tax;

    $("#modal-delete-label").html('Delete Data : ' + data.wht_tax_type + '-' + data.wht_tax_code);
   ...
   $('#button-delete').on( 'click', function () {
    ...
    $.ajax({
      url       : baseURL + 'master/delete_pph/' + id_delete,
      type      : 'GET',
      beforeSend  : function(){
        customLoading('show');
      },
   ...
   $("#btnPrint").on("click", function(){
    var url   ="<?php echo site_url(); ?>Master/cetak_data_pph";
