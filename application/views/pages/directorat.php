          ...
          <table width="100%" class="table display table-bordered table-striped table-responsive" id="table_data"> 
            <thead>
              <tr>
                ...
                <th class="text-center">DIRECTORATE CODE</th>
        ...
        <div class="modal-body"> 
          <div class="row">
            <div class="form-group">
              <div class="col-lg-12">
                <div class="col-lg-8">
                  <label class="form-control-label" for="lblDirectoratCode">Directorat Code</label>
                  <input type="text" class="form-control" id="txtDirectoratCode" name="txtDirectoratCode" placeholder="Directorat Code" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                  <div class="help-block with-errors"></div>
                  <input type="hidden" name="isNewRecord" id="isNewRecord" val="0">
                  <input type="hidden" name="id_dir_code" id="id_dir_code">
                </div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="form-group">
              <div class="col-lg-12">
                <div class="col-lg-8">
                  <label class="form-control-label" for="lblDirectoratName">Directorat Name</label>
                  <input type="text" class="form-control" id="txtDirectoratName" name="txtDirectoratName" placeholder="Directorat Name" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required/>
                  <div class="help-block with-errors"></div>
                </div>
              </div>
            </div>
...
<script>
  $(document).ready(function () {
    ...
    let url = baseURL + 'master/load_data_directorat';
    let ajaxData = {
            "url"  : url,
            "type" : "POST"
          }
    let jsonData = [
                    ...
                    { "data": "directorat_code", "width":"100px", "class":"text-left"},
    ...
    $('#form-edit').validator().on('submit', function(e) {
      if (!e.isDefaultPrevented()){
        $.ajax({
          url     : baseURL + 'master/save_directorat',
          type    : "POST",
    ...
    $('#table_data').on( 'click', 'a.action-edit', function () {
      ...
      $("#isNewRecord").val("0");
      $("#id_dir_code").val(data.id_dir_code);
      $("#txtDirectoratCode").val(data.directorat_code);
      $("#txtDirectoratName").val(data.directorat_name);
      $("#modal-edit-label").html('Edit : ' +  data.directorat_name + '( ' + data.directorat_code + ' )');
    ...
    $('#table_data ').on( 'click', 'a.action-delete', function () {
      ...
      data      = table.row( $(this).parents('tr') ).data();
      id_delete = data.id_dir_code;

      $("#modal-delete-label").html('Delete Data : ' + data.directorat_name + '( ' + data.directorat_code + ' )');
  ...
  $('#button-delete').on( 'click', function () {
    ...
    $.ajax({
      url       : baseURL + 'master/delete_directorat/' + id_delete,
      type      : 'GET',
      beforeSend  : function(){
        customLoading('show');
      },
   ...
   $("#btnPrint").on("click", function(){
    var url   ="<?php echo site_url(); ?>Master/cetak_data_directorat";
