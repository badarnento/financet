      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="navbar-right">
            <button id="btnAdd" class="btn btn-success btn-rounded custom-input-width" data-toggle="modal" data-target="#modal-edit" type="button" ><i class="fa fa-pus"></i> Add New</button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="modal-body"> 
          ...
          <div class="row">
            <div class="form-group">
              <div class="col-lg-12">
                <div class="col-lg-8">
                  <label class="form-control-label" for="lblParentAccount">Parent Account</label>
                  <input type="text" class="form-control" id="txtParentAccount" name="txtParentAccount" placeholder="Parent Account" autocomplete="off" data-toggle="validator" data-error="Please fill out this field" required />
                  <div class="help-block with-errors"></div>
                  <input type="hidden" name="isNewRecord" id="isNewRecord" val="0">
                  <input type="hidden" name="id_group_rpt" id="id_group_rpt">
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="col-lg-12">
              <div class="col-lg-8">
                ...
                <select class="form-control" id="ddlGroupReport" name="ddlGroupReport" data-toggle="validator" data-error="Please choose one" required>
                  <option value=""> Choose GroupReport </option> 
                </select> 
                <div class="help-block with-errors"></div>
              </div>
            </div>
...
<script>
  $(document).ready(function () {
    ...
    let url = baseURL + 'master/load_data_group_rpt';
    let ajaxData = {
      "url"  : url,
      "type" : "POST",
      "data"    : function ( d ) {
    ...
    { "data": "parent_account", "width":"70px", "class":"text-left"},
   ...
   $('#form-edit').validator().on('submit', function(e) {
    if (e.isDefaultPrevented()) {
     ...
     $.ajax({
      url     : baseURL + 'master/save_group_rpt',
      type    : "POST",
   ...
   $('#table_data').on( 'click', 'a.action-edit', function () {
    ...
    $("#isNewRecord").val("0");
    $("#id_group_rpt").val(data.id_group_rpt);
    $("#txtParentAccount").val(data.parent_account);
    $("#ddlGroupReport").val(data.id_group);
    $("#modal-edit-label").html('Edit : ' +  data.group_report + '( ' + data.parent_account + ' )');
   ...
   $('#table_data ').on( 'click', 'a.action-delete', function () {
    ...
    data      = table.row( $(this).parents('tr') ).data();
    id_delete = data.id_group_rpt;


    $("#modal-delete-label").html('Delete Data : ' + data.group_report + '( ' + data.parent_account + ' )');
   ...
   $('#button-delete').on( 'click', function () {
    ...
    $.ajax({
      url       : baseURL + 'master/delete_group_rpt/' + id_delete,
      type      : 'GET',
      beforeSend  : function(){
        customLoading('show');
      },
  ...
  $("#btnPrint").on("click", function(){
    var url   ="<?php echo site_url(); ?>Master/cetak_data_group_rpt";
