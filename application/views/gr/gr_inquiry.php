      ...
      <div class="col-md-2">
        <div class="form-group">
          ...
          <select class="form-control" id="status">
            ...
            <?php foreach ($gr_status as $key => $value): ?>
      ...
      <div class="col-md-4">
        <div class="form-group">
          ...
          <div class="input-group">
            <input class="form-control input-daterange-datepicker" type="text" id="gr_date" value="<?= dateFormat(strtotime('-3 days'), 'date') ?> - <?= dateFormat(time(), 'date') ?>"/>
...
<script>
  ...
  const c_gr_date     = readCookie('gr_date');
  const line_selected = readCookie('gr_line_selected');
  let status_disabled = '';
  ...
  let unit            = $("#unit").val();
  let gr_status       = $("#status").val();
  let gr_date         = $("#gr_date").val();
                   
  let url        = baseURL + 'general-receipt/api/load_gr_header';
  ...
  Pace.track(function(){
      $('#table_data').DataTable({
        ...
        "columns"         : [
                      ...
                      { "data": "division", "width": "140px"},
                      { "data": "unit", "width": "150px" },
                      { "data": "gr_number", "width": "150px"},
                      { "data": "po_number", "width": "80px"},
                      ...
                      { "data": "status", "width": "100px"},
                       { "data": "gr_header_id",
                        ...
                        "render": function (data) {
                           ...
                           <?php if(in_array("GR Inquiry", $group_name)) : ?>
                            $hasil = $hide;
                            return $hasil;
  ...
  $("#tbl_search").on('keyup', "input[type='search']", function(){
   ...
   $('#table_data tbody').on( 'click', 'tr', function () {
    ...
    $("#gr_date").bind("click", function(e) {
      change_date = true;
    }).bind("change", function(e) {
      if(change_date){
        eraseCookie("gr_date");
        createCookie("gr_date", $(this).val());
      }
    });

    $('#table_data').on('click', 'a.action-view', function () {
      gr_header_id = $(this).data('id');
      get_table    = table.row( $(this).parents('tr') );
      index        = get_table.index();
      eraseCookie("gr_line_selected");
      createCookie("gr_line_selected", index);
      $(location).attr('href', baseURL + 'general-receipt/' + gr_header_id);
    });
    ...
    $('#table_data').on('click', 'a.action-edit', function () {
        ...
        // }else{
          eraseCookie("gr_line_selected");
          createCookie("gr_line_selected", index);
          $(location).attr('href', baseURL + 'general-receipt/edit/' + gr_header_id);
        // }
    });
    ...
    $('#table_data').on('click', 'a.action-delete', function () {
      ...
      // if(status.toLowerCase() == "rejected" || status.toLowerCase() == "returned"){
        $("#modal-delete-label").html('Delete Data : ' +  data.gr_number);
    ...
    $('#button-delete').on( 'click', function () {
        ...
        $.ajax({
          url       : baseURL + 'general-receipt/api/delete_gr',
          type      : 'post',
          data      : { id: id_delete },
          beforeSend  : function(){
                          customLoading('show');
    ...
    $('#btn-create').on( 'click', function () {
      customLoading('show');
      setTimeout(function(){
        $(location).attr('href', baseURL + 'general-receipt/create');
      }, 100);
    });

    $("#btnPrint").on("click", function(){
      url   = 'general-receipt/api/download_gr/';

      let params  = { 'id_dir_code' : $("#directorate").val(), 'id_division' : $("#division").val(), 'id_unit' : $("#unit").val(), 'status' : $("#status").val(), 'gr_date' : $("#gr_date").val() };
    ...
    $('#table_data').on('click', 'a.action-cetak', function () {
      let gr_header_id = $(this).data('id');
      var url   = baseURL + 'general-receipt/print';
      if (!table.data().any()){
         customNotif('Info','Data Kosong!','warning' );
         exit();
      }else{
        window.open(url+'/'+gr_header_id, '_blank');
        window.focus();
      }
    });
