  ...
  $(document).ready(function(){
	...
	$('#addRow-detail').on( 'click', function () {
		...
		if($("#rkap_desc-"+indexNow_detail).val() == 0 || $("#nominal_detail-"+indexNow_detail).val() == 0){
			...
			let type_opt     = '<div class="form-group m-b-0"><select id="type_opt-'+numDetail+'" class="form-control input-sm type_opt select-center select2">';
					type_opt += opt_type;
					type_opt += '</select></div>';
			// let uom      = '<div class="form-group m-b-0"><input id="uom-'+numDetail+'" class="form-control input-sm uom"></div>';
    ...
    $("#save_data").on('click', function () {
		...
		else if (fatalNominal == true)
		{
		    ...
		    $.ajax({
		        url   : baseURL + 'purchase-requisition/api/save_pr_edit',
		        type  : "POST",
		        data  : data,
		        dataType: "json",
		        beforeSend  : function(){
	...
	function setTypeVal(id_row, id_select="") {
		$("#type_opt-"+id_row).val(id_select);
	}
