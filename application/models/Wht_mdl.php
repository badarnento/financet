...
class Wht_mdl extends CI_Model {

	public function get_wht($year,$month){
		...
		$mainQuery = " SELECT '' EXPENSE_TYPE,
					GH.NATURE,
					(SELECT DESCRIPTION FROM MASTER_COA WHERE NATURE = GH.NATURE) COA_DESCRIPTION,
					(SELECT
					        GAT.ACCOUNT_DESCRIPTION
					    FROM
		...
		$total = $this->db->query($mainQuery, array($year,$month_new))->num_rows();
		$data  = $this->db->query($queryData, array($year,$month_new))->result_array();
