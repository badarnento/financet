...
class Userapprovedjournal extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Userapprovedjournal_mdl', 'user_approved_journal');
	...
	public function user_approved_journal($batch_approval ="")
	{
		...
		if($this->ion_auth->is_admin() == true || in_array("userapprovedjournal/user-approved-journal", $this->session->userdata['menu_url']) ){
			
			$data['title']          = "User Approved Journal";
			$data['module']         = "datatable";
			$data['template_page']  = "pages/user_approved_journal";
			...
			$breadcrumb[] = array( "name" => "User Approved Journal", "link" => "", "class" => "active" );
	...
	function load_ddl_Approve_batch()
	{
		...
		foreach($query->result_array() as $row)	{
			$result .= "<option value='".$row['BATCH_APPROVAL_LEAD']."' data-name='".$row['BATCH_APPROVAL_LEAD']."' >".$row['BATCH_APPROVAL_LEAD']."</option>";
		}		
		echo $result;
		$query->free_result();
	...
	public function load_data_inquiry(){
		...
		if($total > 0){
			...
			foreach($data as $value) {
				...
				/*if($value['APPROVED'] == 'Y')
				{
					// $checkbox = '<div class="checkbox checkbox-danger"><input id="checkbox-'.$replacejurnal.'" class="checkbox-data checklist" type="checkbox" name="checkbox-data" value='.$value['APPROVED'].' checked><label class="m-0 p-0" for="checkbox-'.$replacejurnal.'"></label></div>';

					$checkbox = '<div class="checkbox form-group m-b-0"><select id="checkbox-'.$replacejurnal.'" name="checkbox-data" class="checklist" data-id="'.$replacejurnal.'" class="form-control input-sm unit_opt select-center" '.$disabled.'>';
					$checkbox .= '<option value="0" >-- Choose --</option>';
					$checkbox .= '<option value="1" '.$selected.'> APPROVED </option>';
					$checkbox .= '<option value="2" >REJECTED</option>';

					$checkbox .=  '</select></div>';
				}
				else if($value['APPROVED'] == 'N'  &&  $value['APPROVED_DATE'] == NULL)
				{
					// $checkbox = '<div class="checkbox checkbox-danger"><input id="checkbox-'.$replacejurnal.'" class="checkbox-data checklist" type="checkbox" name="checkbox-data"  value='.$value['APPROVED'].'><label class="m-0 p-0" for="checkbox-'.$replacejurnal.'"></label></div>';

					$checkbox = '<div class="checkbox form-group m-b-0"><select id="checkbox-'.$replacejurnal.'" name="checkbox-data" class="checklist" data-id="'.$replacejurnal.'" class="form-control input-sm unit_opt select-center" '.$disabled.'>';
					$checkbox .= '<option value="0" '.$selected.'>-- Choose --</option>';
					$checkbox .= '<option value="1" > APPROVED </option>';
					$checkbox .= '<option value="2"> REJECTED </option>';
				...
				else
				{
					...
					$checkbox .= '<option value="0" >-- Choose --</option>';
					$checkbox .= '<option value="1" > APPROVED </option>';
					$checkbox .= '<option value="2" '.$selected.'> REJECTED </option>';

					$checkbox .=  '</select></div>';	
				}*/
				//endregion

				$remark = '<textarea id="remark-'.$replacejurnal.'" class="form-control remark" rows="3" disabled>'.$value['REMARK_APPROVED'].'</textarea>';

				$remark2 = '<textarea id="remark2-'.$replacejurnal.'" class="form-control remark2" rows="3" disabled>'.$value['REMARK_VERIFICATED'].'</textarea>';
				...
				$row[] = array(
					...
					'status'			  => $checkbox,
					'remark_approved'	  => $remark,
					'remark_verificated'  => $remark2,
					'user_login'		  => $user_login,
					'is_more_than_ap'	  => $checkbox2,
					'batch_approval'	  => $value['BATCH_APPROVAL_LEAD']
				);
	...
	public function load_data_journal_after_tax(){
		...
		// $get_all = $this->user_approved_journal->get_journal_after_tax_datatable($invoice_date_from, $invoice_date_to);
		//endregion
		...
		if($total > 0){
			...
			foreach($data as $value) {
				...
				// 	$checkbox = '<div class="checkbox form-group m-b-0"><select id="checkbox-'.$replacejurnal.'" data-id="'.$replacejurnal.'" name="checkbox-data" class="checklist" class="form-control input-sm unit_opt select-center">';
				// 	$checkbox .= '<option value="0" >-- Choose --</option>';
				// 	$checkbox .= '<option value="1" '.$selected.'> APPROVED </option>';
				// 	$checkbox .= '<option value="2" >REJECTED</option>';
				...
				// 	$checkbox = '<div class="checkbox form-group m-b-0"><select id="checkbox-'.$replacejurnal.'" name="checkbox-data" class="checklist" data-id="'.$replacejurnal.'" class="form-control input-sm unit_opt select-center">';
				// 	$checkbox .= '<option value="0" '.$selected.'>-- Choose --</option>';
				// 	$checkbox .= '<option value="1" > APPROVED </option>';
				// 	$checkbox .= '<option value="2"> REJECTED </option>';
				...
				$remark = '<textarea id="remark-'.$replacejurnal.'" class="form-control remark" rows="3">'.$value['REMARK_APPROVED'].'</textarea>';
	...
	function download_data_after_tax()
	{
		...
		foreach($hasil->result_array() as $row)	{
			...
			$excel->setActiveSheetIndex(0)->setCellValue('P'.$numrow, $row['APPROVED']);
		...
		header('Content-Disposition: attachment; filename="User Approved Journal.xls"'); // Set nama file excel nya
	...
	function update_user_approved_journal()
	{
		...
		foreach ($data_line as $key => $value) 
		{
			...
			if($approvedval == "N")
			{
				...
				$data[] = array(
					...
					"VERIFICATED" 				=> $value['status'],
					"REMARK_APPROVED"    		=> $value['remark_approved'],
			...
			{
				$data[] = array(
					"BATCH_APPROVAL_HOU"		=> $batch_approval,
					"NO_JOURNAL" 				=> $value['nojournal'],
					"APPROVED" 					=> $value['status'],
					"VERIFICATED" 				=> "Y",
					"REMARK_APPROVED"    		=> $value['remark_approved'],
		...
		{
			if($dataapprove){
				log_message('info', 'successfully approved');
				$get_user_approval_journal = get_user_approval_ap("HOU");
				if($get_user_approval_journal){
					foreach ($get_user_approval_journal as $key => $value) {
			...
			if($datareject){
				log_message('info', 'successfully rejected');
	...
	{		
		...
		$result .= "<option value='1' data-name='APPROVED_DATE' > Approved Date </option>";
	...
	private function _email_verification($recipient, $batch_approval, $validatedval){
		...
		if($validatedval == 'Y')
		{
			$get_gl = $this->user_approved_journal->get_journal_to_approvehou($batch_approval)->result_array();
		...
		$data['link'] = base_url("user-approved-journalhou/").$encrypt_batch_approve;
		$data['link_display'] = "Financetools User Approved Journal Hou".$batch_approval;
		...
		{
			$get_gl = $this->user_approved_journal->get_journal_to_verification($batch_approval)->result_array();
...
/* End of file Userapprovedjournal.php */

/* Location: ./application/controllers/Userapprovedjournal.php */