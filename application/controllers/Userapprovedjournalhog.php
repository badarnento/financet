...
class Userapprovedjournalhog extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Userapprovedjournalhog_mdl', 'user_approved_journal');
	...
	public function user_approved_journal($batch_approval ="")
	{
		...
		if($this->ion_auth->is_admin() == true || in_array("Userapprovedjournalhog/user_approved_journal", $this->session->userdata['menu_url']) ){
			$data['title']          = " Approval HoG ";
			$data['module']         = "datatable";
			$data['template_page']  = "pages/user_approved_journal_hog";
			...
			$breadcrumb[] = array( "name" => "Approval Hog", "link" => "", "class" => "active" );
	...
	function load_ddl_Approve_batch()
	{
		...
		foreach($query->result_array() as $row)	{
			$result .= "<option value='".$row['BATCH_APPROVAL_HOG']."' data-name='".$row['BATCH_APPROVAL_HOG']."' >".$row['BATCH_APPROVAL_HOG']."</option>";
		}		
		echo $result;
		$query->free_result();
	...
	public function load_data_inquiry(){
		...
		if($total > 0){
			...
			foreach($data as $value) {
				...
				$remark = '<textarea id="remark-'.$replacejurnal.'" class="form-control remark" rows="3" disabled>'.$value['REMARK_APPROVED_HOG'].'</textarea>';

				$remark2 = '<textarea id="remark2-'.$replacejurnal.'" class="form-control remark2" rows="3" disabled>'.$value['REMARK_APPROVED_HOU'].'</textarea>';
				...
				$row[] = array(
					...
					'status'			  => $checkbox,
					'status'			  => $checkbox,
					'remark_approved_hog'	  => $remark,
					'remark_approved_hou'  => $remark2,
					'user_login'		  => $user_login,
					'is_more_than_ap'	  => $checkbox2,
					'batch_approval'	  => $value['BATCH_APPROVAL_HOG']
				);
	...
	public function load_data_journal_after_tax(){
		...
		if($total > 0){
			...
			foreach($data as $value) {
				...
				$remark = '<textarea id="remark-'.$replacejurnal.'" class="form-control remark" rows="3">'.$value['REMARK_APPROVED_HOG'].'</textarea>';
	...
	function download_data_after_tax()
	{
		...
		foreach($hasil->result_array() as $row)	{
			...
			$excel->setActiveSheetIndex(0)->setCellValue('P'.$numrow, $row['APPROVED_HOG']);
		...
		header('Content-Disposition: attachment; filename="Approved Hog.xls"'); // Set nama file excel nya
	...
	function update_user_approved_journal()
	{
		...
		$get_user_verif = get_user_approval_ap("HOU");
		$get_user = NULL;
		$dataapprove = NULL;
		$datareject = NULL;
		...
		foreach ($data_line as $key => $value) 
		{
			...
			if($approvedval == "N")
			{
				...
				$data[] = array(
					...
					"APPROVED_HOG" 				=> $value['status'],
					"APPROVED" 				    => $value['status'],
					"REMARK_APPROVED_HOG"       => $value['remark_approved'],
					"APPROVED_HOG_DATE" 		=> $value['approved_date'],
					"APPROVED_HOU" 				=> NULL
		...
		if($update)
		{
			...
			if($get_user){
				foreach ($get_user as $key => $value) {
					...
					$this->_email_verification($recipient, $batch_approval);
				}
			}
			}
	...
	private function _email_verification($recipient, $batch_approval){

		$action_name = get_user_data($this->session->userdata('user_id'));

		$data['email_recipient']  = $recipient['name'];
		
			$get_gl = $this->user_approved_journal->get_journal_to_verification_reject($batch_approval)->result_array();
...
/* End of file Userapprovedjournalhog.php */

/* Location: ./application/controllers/Userapprovedjournalhog.php */